    ...
    @if ($step == 1)
        <form wire:submit.prevent="openForm">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4  w-full gap-5">
                <x-shared.image-radio id="kitchen_type_straight" name="kitchen_type"
                    src="{{ asset('images/fiddy/kitchen/straight.png') }}" wire:model.live="kitchen_type"
                    value="straight" />
                <x-shared.image-radio id="kitchen_type_parallel" name="kitchen_type"
                    src="{{ asset('images/fiddy/kitchen/parallel.png') }}" wire:model.live="kitchen_type"
                    value="parallel" />
                <x-shared.image-radio id="kitchen_type_lshaped" name="kitchen_type"
                    src="{{ asset('images/fiddy/kitchen/l-shaped.png') }}" wire:model.live="kitchen_type"
                    value="l-shaped" />
                <x-shared.image-radio id="kitchen_type_ushaped" name="kitchen_type"
                    src="{{ asset('images/fiddy/kitchen/u-shaped.png') }}" wire:model.live="kitchen_type"
                    value="u-shaped" />
            </div>
            <div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4  w-full gap-5">
                <x-shared.image-radio id="finish_type_acrylic" name="finish_type"
                    src="{{ asset('images/fiddy/kitchen/acrylic.png') }}" wire:model.live="finish_type"
                    value="acrylic" />
                <x-shared.image-radio id="finish_type_laminate" name="finish_type"
                    src="{{ asset('images/fiddy/kitchen/laminate.png') }}" wire:model.live="finish_type"
                    value="laminate" />
                <x-shared.image-radio id="finish_type_pu" name="finish_type"
                    src="{{ asset('images/fiddy/kitchen/pu.png') }}" wire:model.live="finish_type" value="PU" />
            </div>
            <div>
            ...
            @foreach ($typeSides[$kitchen_type] as $s)
                ...
                <div class="grid grid-cols-3 gap-3 sm:gap-5">
                    <div class="flex flex-col gap-1">
                        ...
                        <x-shared.text-input class="text-sm !bg-white" id="length{{ $s }}" type="number"
                            required step="0.1" min="1" name="length{{ $s }}"
                            wire:model="length_{{ strtolower($s) }}" placeholder="Length" />
                        <x-input-error for="length_{{ strtolower($s) }}" class="mt-2" />
                    </div>
                    <div class="flex flex-col gap-1">
                        <label for="breadth{{ $s }}" class="text-sm">Slab Depth (ft)</label>
                        <x-shared.text-input class="text-sm" id="breadth{{ $s }}" type="number" disabled
                            required step="0.1" min="1" name="breadth{{ $s }}"
                            wire:model="breadth_{{ strtolower($s) }}" placeholder="Breadth" />
            ...
            <form wire:submit.prevent="save">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-5 mt-5">
                    <div class="flex flex-col gap-1">
                        ...
                        <x-shared.text-input class="text-sm !bg-white" id="name" type="text" name="name"
                            wire:model.live="name" placeholder="Enter your name" />
                        <x-input-error for="name" class="mt-2" />
                    </div>
                    <div class="flex flex-col gap-1">
                        <label for="email" class="text-sm">Email</label>
                        <x-shared.text-input class="text-sm !bg-white" id="email" type="email" name="email"
                            wire:model.live="email" placeholder="Enter your email" />
                        <x-input-error for="email" class="mt-2" />
                    </div>
                    <div class="flex flex-col gap-1">
                        <label for="phone" class="text-sm">Phone</label>
                        <x-shared.text-input class="text-sm !bg-white" id="phone"
